...
class Cell(Agent):
    ...
    #         "local_geometric_frustration" : self.local_geometrical_frustration(),
    #         "bias" : 0.5,
    ...
    # def get__molecules(self):
    #         return self.molecules
    #     def get__energy(self):
    #         return self.energy
    #     def get__energyt1(self):
    #         return self.energyt1
    #     def get__stress(self):
    #         return self.stress
    #     def get__stresst1(self):
    #         return self.stresst1
    #     def get__state(self):
    #         return self.state
    #     def get__statet1(self):
    #         return self.statet1
    #     def get__local_geometrical_frustration(self):
    #         return self.local_geometrical_frustration()
    def bias(self):
        return 0.5
    def get__collective_size(self):
        return self.collective_size
    def get__french_flag(self):
        return np.abs(100 - (self.collective_size/(self.model.nb_goal_cells/3)*100)) 
    ...
    def net_input(self, perc_blue, perc_red, perc_white, fitness_ff, tissue_matrix):
        ...
        # #         "bias()",
        # #         ]
        # for input_string in self.model.inputs[1:]:
        # # for input_string in inputs[1:]:
        #     # inp = self.get(input_string)
        #     # inp = eval("self.get__" + input_string + "()")
        #     inp = eval("self." + input_string)
        #     # Handle molecules differently since it is a list
        #     # if (input_string=="molecules"):
        #     #     new_input.append(list(inp))
        #     #     new_input = sum(new_input, [])
        #     # else:
        #     new_input.append(inp)
            
        #Eval is slow, what if just use massive if block?
        if "molecules" in self.model.inputs:
            new_input.append(list(self.molecules))
            new_input = sum(new_input, [])
        if "energy" in self.model.inputs:
            new_input.append(self.energy)
            new_input.append(self.energyt1)
        if "stress" in self.model.inputs:
            new_input.append(self.stress)
            new_input.append(self.stresst1)
        if "state" in self.model.inputs:
            new_input.append(self.state)
            new_input.append(self.statet1) 
        if "local_geometrical_frustration" in self.model.inputs:
            new_input.append(self.local_geometrical_frustration()) 
        if "bias" in self.model.inputs:
            new_input.append(0.5)
        if "collective_size" in self.model.inputs:
            new_input.append(self.collective_size)
        if "french_flag" in self.model.inputs:
