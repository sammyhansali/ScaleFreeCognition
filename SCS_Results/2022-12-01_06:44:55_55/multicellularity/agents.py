...
class Cell(Agent):
    ...
    def update_state(self, energy_temp):
        tot = 0
    ...
    def update_molecs_and_state(self, outputs, energy_temp):
    ...
    def cell_division(self, energy_temp):
        x = self.pos[0]
        y = self.pos[1]
    ...
    def divide(self, pos, energy_temp):
        x,y = pos

        # Updating initial cell
        energy_temp -= 5
        # 1. Should give half of its molecs to daughter?
        # 2. Should lose 5 energy? Daughter should always have 50 energy?
    ...
    def step(self):
        ...
        energy_temp = self.energy[0]

        if energy_temp >= 5 and outputs["cell_division"] == 1:
            self.cell_division(energy_temp)
        ...
        self.update_molecs_and_state(outputs, energy_temp)
        self.update_stress(outputs)
        # E needs to be updated last since the above updates can sap E
        self.energy = self.update_history(self.energy, energy_temp+self.global_fitness[0]-self.model.e_penalty)
