...
def run_experiment(exp):
    ...
    ### Serializing with JSON - Experimental
    exp_file = os.path.join(".", "exp.json")
    with open(exp_file, 'wb') as f:
        json.dump(exp, f)
    os.replace(exp_file, "SCS_Results/" + file + "/" +  "exp.json")
    ##### Below is experimental
    # Seeing if json is fucking up
    with open("SCS_Results/" + file + "/" +  "exp.json", "rb") as pp:
        test_json_exp = json.load(pp)
...
#     ###
    ...
    subprocess.Popen(['python', 'analysis.py'], cwd="SCS_Results/" + file)
