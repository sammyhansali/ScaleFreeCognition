...
def get_elements(height, width):
    cell_types = CanvasGrid(cell_types, height, width, 200, 200)
    # stress = CanvasGrid(stress, height, width, 200, 200) # Potential replaced stress's code
    GJ_opening_ions = CanvasGrid(GJ_opening_ions, height, width, 200, 200)
    # canvas_element7 = CanvasGrid(molecules, height, width, 200, 200)
    potential = CanvasGrid(potential, height, width, 200, 200)
    delta_potential = CanvasGrid(delta_potential, height, width, 200, 200)
    energy = CanvasGrid(energy, height, width, 200, 200)
    ...
    return [cell_types, # Cell States
            potential, # potential t1 (visualized w color)
            # delta_potential, # potential t1-t0
            energy, # 
            # canvas_element9, 
            # stress, # Agent's stress
            GJ_opening_ions, # GJ_opening_ions. Light = More open. Dark = More closed.
