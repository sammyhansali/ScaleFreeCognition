...
class Cell(Agent):
    ...
    def __init__(self, net, depth, unique_id, pos, model, moore, molecules, goal_cell_type, bioelectric_stimulus, GJ_opening_ions, GJ_opening_stress, energy, stress, direction, local_fitness, global_fitness, cell_type, potential):
        """
    ...
    def step(self):
        ...
        if "cell_type" in outputs:
            ...
            if outputs["cell_type"] == 1 and abs(-60 - self.potential[0]) < 5:
                self.differentiate(1)
            elif outputs["cell_type"] == 2 and abs(-50 - self.potential[0]) < 5:
                self.differentiate(2)
            elif outputs["cell_type"] == 3 and abs(-40 - self.potential[0]) < 5:
