...
class Multicellularity_model(Model):
    ...
    def __init__(self,
        ...
        datacollector_dict = {}
            ...
            # }
        # if (self.nb_output_molecules > 0): 
        #     # exchanges = self.schedule.get_nb_molecules_exchanged(Cell) # if 0, just returns tot. El
        self.molecules_exchanged = self.schedule.nb_molecules_exchanged(Cell)      # Do calculations in one go
        print(self.molecules_exchanged)
        for i in range(nb_output_molecules):
            # datacollector_dict[f"Molecule {i} exchanged"] = exchanges[i]
            # datacollector_dict[f"Molecule {i} exchanged"] = lambda m:m.schedule.get_nb_molecules_exchanged(Cell, i)
            datacollector_dict[f"Molecule {i} exchanged"] =  lambda m:m.get_nb_molecules_exchanged(Cell, i)

        if self.nb_output_molecules > 1:
            # datacollector_dict["Total molecules exchanged"] = sum(exchanges)
            # datacollector_dict["Total molecules exchanged"] = lambda m:m.schedule.get_nb_molecules_exchanged(Cell, "total")
            datacollector_dict["Total molecules exchanged"] =  lambda m:m.get_nb_molecules_exchanged(Cell, "total")
