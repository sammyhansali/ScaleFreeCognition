...
def run_experiment(exp):
    ...
    # Testing genome.Save(filepath)
    best_genome[-1].Save("best_genome.txt")
    ...
    with open("best_genome.pickle", 'wb') as f:
        pickle.dump(best_genome[-1], f, pickle.HIGHEST_PROTOCOL)
...
step_count: %s" % (exp.depth, exp.height, exp.width, 
        exp.energy, exp.nb_output_molecules, exp.history_length, exp.e_penalty, exp.step_count), 
...
def eval_individual(genome):
    ...
    if fit1 > 95:
        global file
        net.Save("SCS_Results/" + file + "/winner_net_"+ str(round(fit1,1))+".txt")
