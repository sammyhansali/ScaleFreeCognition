...
class Multicellularity_model(Model):
    ...
    def __init__(self,
        ...
        self.nb_output_molecules = sum(["_to_send" in x for x in self.ANN_outputs])
        self.full_reservoir = 45
        self.depleted_reservoir = self.full_reservoir

        # Create cells on the whole grid
        self.schedule.generate_cells(Cell, self.start)
        print(len(self.grid.get_cell_list_contents([(4,4)])) > 0)
        print(self.grid.get_cell_list_contents([(4,4)])[0].molecules)
