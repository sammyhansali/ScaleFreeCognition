...
class RandomActivationByBreed(RandomActivation):
    ...
    def get_global_stress(self, Cell):
        ...
        for agent_key in agent_keys:
            if self.agents_by_breed[Cell][agent_key].cell_type[0] != self.agents_by_breed[Cell][agent_key].tissue_goal_cell_type[0]:
                stress+=1
        return stress
    ...
    def get_spatial_entropy(self, Cell):
        ...
        for i in range(self.model.height):
            for j in range(self.model.width):
                for agent_key in agent_keys:
                    if self.agents_by_breed[Cell][agent_key].pos[0][0] == j and self.agents_by_breed[Cell][agent_key].pos[0][0] == i:
                        points.append([j,i])
    ...
    def global_fitness(self):
        ...
        curr_dissimilarity = self.dissimilarity_2()
    ...
    def dissimilarity(self):
        ...
        for i in range(self.model.height):
            for j in range(int(self.model.width)):
                ...
                if len(self.model.grid.get_cell_list_contents([(j,i)]))>0:
                    cell = self.model.grid.get_cell_list_contents([(j,i)])[0]
                    if cell.cell_type[0] != cell.tissue_goal_cell_type[0]:
    ...
    # Only looks at non-skin cells for fitness calculation, lets see if helps electric face
    def dissimilarity_2(self):
        curr_dissimilarity=0
        for i in range(self.model.height):
            for j in range(int(self.model.width)):
                # alive cell on spot
                if len(self.model.grid.get_cell_list_contents([(j,i)]))>0:
                    cell = self.model.grid.get_cell_list_contents([(j,i)])[0]
                    if cell.cell_type[0] != 1 and cell.cell_type[0] != cell.tissue_goal_cell_type[0]:
                        curr_dissimilarity += 1
                # nothing on spot
                else:
                    if self.model.goal[i][j] != 0:
        ...
        for y in range(self.model.height):
            for x in range(self.model.width):
                ...
                cell = Cell(    net = self.model.net, 
                                ...
                                cell_type = [0]*h,
                                potential = [0]*h,
                                tissue_goal_cell_type = [0]*h, 
                                pos = (x,y), # Similar to "cell.positional_history" but NOT the same. 
                                positional_history = [(0,0)]*h,  # Use only this for inputs.
                                molecules = molecules, 
                            )
                ...
                cell.tissue_goal_cell_type[0] = self.model.goal[y][x]
                cell.positional_history[0] = (x, y)
