...
class Cell(Agent):
    ...
    cell_type = None
    potential = None
    positional_history = None



    def __init__(self, net, depth, unique_id, pos, model, moore, molecules, positional_history, tissue_goal_cell_type, GJ_opening_ions, GJ_molecules, GJ_opening_stress, energy, stress, direction, global_fitness, cell_type, potential):
        """
        ...
        self.positional_history = positional_history
    ...
    def net_inputs(self):
        ...
        if "pos_x" in self.model.ANN_inputs:
            x_vals = [coord[0] for coord in self.positional_history]
            inputs.extend(x_vals)
        if "pos_y" in self.model.ANN_inputs:
            y_vals = [coord[1] for coord in self.positional_history]
    ...
    def update_potential(self, charge_to_send):
        neighbours = self.model.grid.get_neighborhood(self.positional_history[0], self.moore, False)
    ...
    def cell_motility(self, direction):
        pos_1 = self.positional_history[0]
        pos_2 = self.get_pos_in_direction(pos_1, direction)
        
        if not self.out_of_bounds(pos_2):
            self.switch(pos_1, pos_2)
    ...
    def switch(self, pos_1, pos_2):
        ...
        # if neither empty, switch
            ...
            cic.positional_history = cic.update_history(cic.positional_history, pos_1)
        ...
        self.positional_history = self.update_history(self.positional_history, pos_2)
    ...
    def activity(self, outputs):
        ...
        if "motility" in outputs:
            ...
            self.positional_history = self.update_history(self.positional_history, self.positional_history[0])
        # Division
        if "cell_division" in outputs:
    ...
    def die(self):
        self.model.grid._remove_agent(self.positional_history[0], self)
