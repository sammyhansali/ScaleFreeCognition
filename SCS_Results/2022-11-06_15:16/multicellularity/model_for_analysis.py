...
class Multicellularity_model(Model):
    ...
    def run_model(self, fitness_evaluation):
        ...
        if fitness_evaluation==True:
            ...
            self.fitness_ff=0
            general_energy=0
            # Not using schedule.fitness_ff() because want to measure g_e as well. 
            for i in range(self.height):
            ...
            # Shouldn't be dividing by height*width. If the shape is a cross, you need to get the area differently.
            # Area needs to be calculated from goal matrix from the very beginning.
            # self.fitness_ff = self.fitness_ff/(self.height*self.width)*100
            # general_energy   = general_energy/ (self.height*self.width*10)        
            self.fitness_ff = self.fitness_ff/(self.nb_goal_cells)*100
