...
step_count: %s" % (exp.nb_gens, exp.depth, exp.height, exp.width, 
    ...
    # # save start and goal matrices
    np.savetxt("../SCS_Results/" + file + "/" + "start_mat.txt", g_exp.start_mat)
    np.savetxt("../SCS_Results/" + file + "/" + "goal_mat.txt", g_exp.goal_mat)
    ...
    import subprocess
    subprocess.Popen("python3 analysis.py", cwd="../SCS_Results/" + file + "/", shell=True)
    os.rename('../SCS_Results/' + file,'../SCS_Results/' + file +'_'+ str(int(best_ever_goal_fitness)))
