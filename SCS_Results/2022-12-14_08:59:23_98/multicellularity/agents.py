...
class Cell(Agent):
    ...
    def __init__(self, net, depth, unique_id, pos, model, moore, molecules, goal_cell_type, bioelectric_stimulus, GJ_opening_ions, GJ_opening_molecs, GJ_opening_stress, energy, stress, direction, global_fitness, cell_type, potential):
        """
        ...
        self.GJ_opening_molecs = GJ_opening_molecs
    ...
    def update_potential(self, charge_to_send):
        ...
        for neighbour in neighbours:
            ...
            if self.model.grid.is_cell_empty(neighbour)==False: # not dead
                ...
                GJ_open_percentage = min(self.GJ_opening_ions, cic.GJ_opening_ions)

                new_cic_potential = 0
                if new_self_potential >= charge_to_send * GJ_open_percentage:
                    to_send = charge_to_send * GJ_open_percentage
    ...
    def update_molecs(self, outputs):
        ...
        for neighbour in neighbours:
            ...
            if self.model.grid.is_cell_empty(neighbour)==False: # not dead
                cic = self.model.grid[neighbour][0] # Cell In Contact
                GJ_opening_molecs = min(self.GJ_opening_molecs, cic.GJ_opening_molecs)

                for i in range(self.model.nb_output_molecules):
                    key = f"molecule_{i}_to_send"
                    new_cic_molecs = 0

                    if new_self_molecs[i] >= outputs[key]*GJ_opening_molecs:
                        to_send = outputs[key]*GJ_opening_molecs
    ...
    def step(self):
        ...
        if "cell_type" in outputs:
            ...
            ## No resting potential so no conditionals.
            # # if you want to differentiate, need to be within 5 of the resting potential before permitted
            # epsilon = 15
            # if outputs["cell_type"] == 1 and abs(-60 - self.potential[0]) < epsilon:
            #     self.differentiate(1)
            # elif outputs["cell_type"] == 2 and abs(-50 - self.potential[0]) < epsilon:
            #     self.differentiate(2)
            # elif outputs["cell_type"] == 3 and abs(-40 - self.potential[0]) < epsilon:
            #     self.differentiate(3)
            # # Even tho cell type didn't change, need to update cell_type history.
            # else:
            #     self.cell_type = self.update_history(self.cell_type, self.cell_type[0])
