         ...
         # Create cells on the whole grid
             ...
             for i in range(self.model.height):
                 for j in range(self.model.width):
                     ...
                     cell = Cell(self.model.net, self.model.depth, self.model.next_id(), (j, i), self.model,  True, 
                                 ...
                                 molecules = molecules, goal = self.model.goal[i][j], GJ_opening_molecs = [0,0,0,0], 
             ...
             for i in range(self.model.height):
                 for j in range(self.model.width):
                     ...
                     state_tissue = 1
                     molecules =  [random.random()] #np.array([3.,3.]) #
                    # molecules = [0]*self.model.nb_molecules #I need to make this an attribute of the model!
                     if state_cell == 1:
                         molecules[0] = 11
                     elif state_cell == 2:
                         molecules[0] = 3
                     ...
                     cell = Cell(self.model.net, self.model.depth, self.model.next_id(), (j, i), self.model,  True, 
                                 ...
                                 molecules = molecules, goal = self.model.goal[i][j], GJ_opening_molecs = [0,0,0,0], 
                                 GJ_opening_stress=0, stress = 0, stresst1=0, decision_state0=0, decision_state1=0,decision_state2=0, 
                                 state=state_cell, statet1=state_cell, state_tissue = state_tissue, m0_GJ_opening=0)
                     self.model.grid.place_agent(cell, (j, i))
                     self.model.schedule.add(cell)
