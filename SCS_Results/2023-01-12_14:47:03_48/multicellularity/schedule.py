...
class RandomActivationByBreed(RandomActivation):
    ...
    def change_gap_junctions(self, GJ_to_change, matrix):
        for y in range(self.model.height):
            for x in range(int(self.model.width)):
                # alive cell on spot
                if len(self.model.grid.get_cell_list_contents([(x,y)]))>0:
                    cell = self.model.grid.get_cell_list_contents([(x,y)])[0]
                    cell.GJ_molecules[GJ_to_change] = matrix[y][x] # Add history later
        ...
        for y in range(self.model.height):
            for x in range(self.model.width):
                ...
                for n in range(self.model.nb_output_molecules):
                    ...
                    molecules[n][1] = 5     # Arbitrary amount of signaling molecules to begin with
                    GJ_molecules[n] = 0
