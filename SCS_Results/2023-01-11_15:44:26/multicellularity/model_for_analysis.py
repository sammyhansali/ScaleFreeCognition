...
class Multicellularity_model(Model):
    ...
    def __init__(self,
        ...
        datacollector_dict = {}
            ...
            # }
        functions = [   
                        lambda m:m.schedule.get_nb_molecules_exchanged_0, 
                        lambda m:m.schedule.get_nb_molecules_exchanged_1, 
                        lambda m:m.schedule.get_nb_molecules_exchanged_2, 
                        lambda m:m.schedule.get_nb_molecules_exchanged_3,
                        lambda m:m.schedule.get_nb_molecules_exchanged_4,
                    ]
        for i in range(nb_output_molecules):
            if i==0:
                datacollector_dict["Molecule 0 exchanged"] = lambda m:m.functions[0](Cell)
            elif i==1:
