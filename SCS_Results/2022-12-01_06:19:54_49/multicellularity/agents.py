...
class Cell(Agent):
    ...
    def divide(self, pos, subtract):
        ...
        self.energy =  self.update_history(self.energy, self.energy[0]-10)
        # 1. Should give half of its molecs to daughter?
        # 2. Should lose 10 energy? Daughter should always have 50 energy?
    ...
    def step(self):
        ...
        if self.energy[0] >= 10 and outputs["cell_division"] == 1:
            self.cell_division()
        ...
        # Updating variables
        self.energy = self.update_history(self.energy, self.energy[0]+self.global_fitness[0]-self.model.e_penalty)
