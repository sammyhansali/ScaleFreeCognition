...
class Cell(Agent):
    ...
    def __init__(self, net, depth, unique_id, pos, model, moore, molecules, goal, GJ_opening_ions, GJ_opening_stress, energy, stress, state, direction, local_fitness, global_fitness, cell_type, potential):
        """
    ...
    def update_potential(self, charge_to_send):
        ...
        self.potential = self.update_history(self.potential, new_potential)
    ...
    def step(self):
        ...
        if "cell_type" in outputs:
            ...
            if outputs["cell_type"] == 1 and abs(-60 - self.molecules[0]) < 5:
                self.differentiate(1)
            elif outputs["cell_type"] == 2 and abs(-50 - self.molecules[0]) < 5:
                self.differentiate(2)
            elif outputs["cell_type"] == 3 and abs(-40 - self.molecules[0]) < 5:
                self.differentiate(3)
            # Even tho cell type didn't change, need to update history.
            else:
