...
class Cell(Agent):
    ...
    #             self.birth(cell, x, y)
    def cell_division(self, output_prob, updated_reservoir):
        neighbours = self.model.grid.get_neighborhood(self.pos, self.moore, False)
        n = len(neighbours)
    ...
    def step(self, depleted_reservoir, full_reservoir):
        ...
        if "cell_division" in outputs:
            updated_reservoir = self.cell_division(outputs["cell_division"], updated_reservoir)
