...
class Cell(Agent):
    ...
    def communication(self):
        ...
        # print("cell_division" in output) #prints false! why?
        # print(output.keys()) # apop isn't shown here. But in ANN_outputs it is. Why?
        ...
        if len(dead) > 0:
            ...
            if probability == 1:
                ...
                cell = Cell(self.net, self.depth, self.model.next_id(), (x,y), self.model,  True, 
                                self.molecules, self.energy, None, self.cell_gain_from_good_state,  self.model.goal[y][x], 
                                self.GJ_opening_molecs, self.GJ_opening_stress, self.stress, None, self.decision_state0, self.decision_state1, 
                                self.decision_state2, self.state, None, self.state_tissue)
                self.birth(cell, x, y)
