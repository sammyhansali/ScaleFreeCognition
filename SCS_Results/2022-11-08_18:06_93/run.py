...
step_count: %s" % (exp.depth, exp.height, exp.width, 
        exp.energy, exp.nb_gap_junctions, exp.step_count), 
    ...
    # Save Experiments folder
    # shutil.copytree('./Experiments', "SCS_Results/" + file + "/" + "multicellularity")
    ...
    # Saving goal and start matrices
    # np.savetxt(g_exp.goal)
    # np.savetxt(g_exp.start)
    ...
    # save the entire experiment instance
    exp_file = os.path.join(".", "exp.pickle")
    with open(exp_file, 'wb') as f:
        pickle.dump(exp, f)
    # f.close() 
    os.replace(exp_file, "SCS_Results/" + file + "/" +  "exp.pickle")
    # pickle needs me to also save the experiment class
