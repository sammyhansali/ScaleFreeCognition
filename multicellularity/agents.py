...
class Cell(Agent):
    ...
    def update_cell_type(self, mode):
        # This method doesn't work for mode 6, since mode 6 requires an additional input to the function.
        functions = [self.update_cell_type_mode_1, self.update_cell_type_mode_2, self.update_cell_type_mode_3, self.update_cell_type_mode_4, self.update_cell_type_mode_5]
        new_cell_type = functions[mode-1]()
    ...
    def step(self):
        ...
        if "molecules" in self.model.ANN_inputs:
            ...
            # Electric face 6 modes
            if self.model.ef_mode == 6:
                self.update_cell_type_mode_6(outputs["differentiate"])
            else:
                self.update_cell_type(self.model.ef_mode)
            #     self.update_cell_type_with_molecs()
            # elif self.model.ef_mode == 2:
            #     self.update_cell_type_mode_2() 
            # elif self.model.ef_mode == 3:
            #     self.update_cell_type_mode_3()
